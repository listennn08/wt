...
export class RemoveCommand extends AbstractCommand {
  ...
  public load(program: Command) {
    program
      ...
      .action(async (target: string, options: { force?: boolean; branch?: string; path?: string }) => {
        await this.removeWorktreeTarget(target, options);
      });
  }
  ...
  private async removeWorktreeTarget(
    target: string,
    options: { force?: boolean; branch?: string; path?: string },
  ): Promise<void> {
    ...
    await this.git.raw(force ? ['worktree', 'remove', '--force', resolved] : ['worktree', 'remove', resolved]);
